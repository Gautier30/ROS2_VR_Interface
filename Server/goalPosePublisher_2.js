...
rclnodejs.init().then(() => {
    const node = rclnodejs.createNode('robot_goal_node', 'waffle2');
    const publisher = node.createPublisher('geometry_msgs/msg/PoseStamped', '/waffle2/goal_pose');
    ...
    socket.addEventListener("message", (event) => {
        if(!(event.data instanceof Blob) && typeof event.data != "object"){
            ...
            if (msgParsed['identifier'] === 'waffle2/goal_pose') {
