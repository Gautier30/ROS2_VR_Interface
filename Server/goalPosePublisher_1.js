...
rclnodejs.init().then(() => {
    const node = rclnodejs.createNode('robot_goal_node', 'waffle1');
    const publisher = node.createPublisher('geometry_msgs/msg/PoseStamped', '/waffle1/goal_pose');
    ...
    socket.addEventListener("message", (event) => {
        if(!(event.data instanceof Blob) && typeof event.data != "object"){
            ...
            if (msgParsed['identifier'] === 'waffle1/goal_pose') {
