...
rclnodejs.init().then(() => {
    const node = rclnodejs.createNode('cmdVel_VR_node', 'waffle1');
    const publisher = node.createPublisher('geometry_msgs/msg/Twist', '/waffle1/cmd_vel');
    ...
    socket.addEventListener("message", (event) => {
        if(!(event.data instanceof Blob) && typeof event.data != "object"){
            ...
            if (msgParsed['identifier'] === 'waffle1/lin_vel' || msgParsed['identifier'] === 'waffle1/ang_vel'){
                
                if (msgParsed['identifier'] === 'waffle1/lin_vel') {
                ...
                if (msgParsed['identifier'] === 'waffle1/ang_vel') {
