...
rclnodejs.init().then(() => {
    const node = rclnodejs.createNode('cmdVel_VR_node', 'waffle3');
    const publisher = node.createPublisher('geometry_msgs/msg/Twist', '/waffle3/cmd_vel');
    ...
    socket.addEventListener("message", (event) => {
        if(!(event.data instanceof Blob) && typeof event.data != "object"){
            ...
            if (msgParsed['identifier'] === 'waffle3/lin_vel' || msgParsed['identifier'] === 'waffle3/ang_vel'){
                
                if (msgParsed['identifier'] === 'waffle3/lin_vel') {
                ...
                if (msgParsed['identifier'] === 'waffle3/ang_vel') {
