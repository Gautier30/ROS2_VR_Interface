...
socket.addEventListener("open", (event) => {
    ...
    rclnodejs.init().then(() => {
        ...
        const node = rclnodejs.createNode('robot_camera_node', 'waffle1');

        socket.addEventListener("message", (event) => {
            if(!(event.data instanceof Blob) && typeof event.data != "object"){
                const receivedMessage = JSON.parse(event.data);

                if(receivedMessage.identifier === "waffle1/camera_trigger"){

                    if(receivedMessage.command === "capture_ON"){
                        console.log("Capture..");
                        

                        camSubscriber = node.createSubscription('sensor_msgs/msg/Image', '/waffle1/camera/image_raw', (msg) => {
